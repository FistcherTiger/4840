...
theta_t cal_joint_angle(float x, float y, float z) {
   theta_t thetas;
    ...
    float l2 = 0.209;
    z = z - 0.0955;
    thetas.t0 = 202 - (acosf((powf(l1, 2) + powf(l2, 2) - powf(y, 2) - powf(z, 2)) / (2 * l1 * l2)))/3.14*180 - (atanf(z / (y + 0.000001)))/3.14*180 - (acosf((powf(y, 2) + powf(z, 2) + powf(l2, 2) - powf(l1, 2)) / (2 * l2 * sqrtf(powf(y, 2) + powf(z, 2)))))/3.14*180;

    thetas.t1 = (atanf(z / y))/3.14*180 + (acosf((powf(y, 2) + powf(z, 2) + powf(l2, 2) - powf(l1, 2)) / (2 * l2 * sqrtf(powf(y, 2) + powf(z, 2)))))/3.14*180;

    thetas.t2 = (atanf(y / (x + 0.000001)))/3.14*180;
    printf("before");
    printf("%f %f %f\n",thetas.t0, thetas.t1, thetas.t2);
    return thetas;
}
...
int main()
{
  ...
  control_servo_t vla;
  int i;
  static const char filename[] = "/dev/servo_control";


   // Create the additional thread
   int result = pthread_create(&additional_thread, NULL, thread_function, NULL);
    if (result != 0) {
        printf("Error creating the additional thread\n");
        return 1;
    }
  ...
  for (;;) 
   {
      ...
      if (transferred == sizeof(packet)) 
      {
         ...
         if(packet.keycode[0] == 0 && (prev_keystate[3] != '0' || prev_keystate[4] != '0')) // Key released, 
         {
            printf("key released\n");
            key_switch = 0;
            is_write = 0;
         ...
         else if(packet.keycode[0] == 29)// z reset the position
         {
